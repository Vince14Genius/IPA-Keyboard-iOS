...
extension IPAKeyboardViewControllerTemplate {
    ...
    @objc func insertKeyButtonText(from button: UIButton, with event: UIEvent) {
        ...
        let cleanedText = Symbols.removedDottedCircles(text)

        // Attempt to handle Thai tone logic
        if ThaiToneHandler.handleToneInput(cleanedText, proxy: textDocumentProxy) {
            return
        }
